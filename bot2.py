...
st.title(f'{emoji_robo} pregunta a Cosme Fulanito')
st.write('***')

if 'hst_conversa' not in st.session_state:
    st.session_state.hst_conversa = []

pregunta = st.text_input('Escriba su pregunta')
...
with col2:
    btn_limpar = st.button("Limpar conversacion")

if btn_enviar: 
    st.session_state.hst_conversa.append({"role": "user", "content": pregunta})
